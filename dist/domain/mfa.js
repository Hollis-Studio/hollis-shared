 ...
 */
export const ASSIGNMENT_STATUSES = ["ACTIVE", "REVOKED", "PENDING"];
...
const mfaLoginPendingUserInputSchema = z
    ...
    if (typeof canonicalUserId !== "string" ||
...
export const mfaLoginPendingUserSchema = mfaLoginPendingUserInputSchema.transform(({ uid, userId, displayName, fullName, ...user }) => ({
    ...user,
    userId: userId ?? uid ?? "",
...
export const backupCodesRequestSchema = z.object({
    ...
    code: z
        .string()
        .length(MFA_TOTP_CODE_LENGTH, `TOTP code must be exactly ${MFA_TOTP_CODE_LENGTH} digits`)
