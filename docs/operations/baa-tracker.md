...
| **Sentry** | **Health runtime flow excluded by policy; deployed versions may still send until replaced** | **NOT SIGNED — free tier retained** | On 2026-09-21 the owner chose to keep the free plan and exclude Health mobile, API, admin, and public-web runtime ingestion. Current source disables each Health initializer, sets zero sampling, and drops events/transactions; new mobile manifests omit the DSN even if EAS supplies one. Build-only release/source-map tooling may remain. The Health ingestion key is intentionally still enabled, so older installed binaries and currently deployed services may continue sending until updated; source changes alone do not prove live exclusion. | Engineering — preserve fail-closed Health configuration and verify each deployed replacement; Isaac — revisit before any runtime re-enable | **High until deployed versions are replaced; then watch** |
...
The 2026-09-21 owner decision is to retain the free plan, which has no BAA, and exclude runtime reporting from every Health surface rather than upgrade. Current Health source uses `enabled: false`, zero trace/replay sampling, and `beforeSend` / `beforeSendTransaction` hooks that return `null`. The mobile build configuration also omits `extra.sentryDsn` when a remote EAS environment still contains `SENTRY_DSN`. Local logs and CloudWatch remain enabled.

Release and source-map credentials are a separate build-time path and may remain configured; they do not authorize runtime events. Redaction helpers remain defense in depth, but are not the basis of this decision.

This is a source-state control, not a live-completion receipt. The existing Health ingestion key remains enabled by explicit owner choice. Older mobile binaries and deployed API/admin/public versions can therefore continue to report until replaced with the excluded configurations. Verify each deployment or delivered binary separately, and do not mark suite-wide exclusion complete from local tests. The Workouts Sentry project is separate and is not changed by the Health decision.
...
Last reviewed: 2026-09-21 (Sentry row updated for the owner decision to retain the free plan and exclude Health runtime ingestion in source. The Health ingestion key remains enabled, so deployed-version replacement and verification are still pending. Other vendor rows retain the 2026-09-20 review.)
