...
## D. Execution status (updated 2026-05-12 PM; status notes added 2026-05-19)

> **2026-05-19 status note:** Items marked ✅ below that were "Pending" as of 2026-05-12 PM have been verified against the current `hollis-shared` codebase. Specifically: the `MuscleGroup` 23-value set (A.1) is confirmed live in `packages/contracts/domain/muscles.ts`; the merged `EquipmentType` superset with `cable` (A.2) is confirmed in `packages/contracts/domain/equipment.ts`; `Result<T>` / `ok()` / `err()` (B.1) are confirmed in `packages/contracts/errors/result.ts`; the `AppErrorCode` superset with scope JSDoc and aliases (B.2) is confirmed in `packages/contracts/errors/app-error-code.ts`; the `bodyWeightKgSchema` / `loadWeightKgSchema` split (B.4) is confirmed in `packages/contracts/schemas/weight.ts`; the Clay palette (C.1) is confirmed in `packages/design-tokens/clay.ts`; `spacingClay` (C.2) is confirmed in `packages/design-tokens/spacing-clay.ts`; and the Workouts-side `ExerciseCategory` → `TrackingMode` rename (A.3) is reflected in `domain/training-session-log.ts` exporting `ExerciseTrackingModeSchema`. Items that remain genuinely pending (not yet in the shared package codebase) are called out below.
...
| Adopt shared `MuscleGroup` (A.1)                     | ✅ Done — 23-value set confirmed in `packages/contracts/domain/muscles.ts`              | Canonical plan §4.5 Step 4                                                                         |
| Firestore `cable_machine` → `cable` migration + adopt shared `EquipmentType` (A.2) | ✅ Done (shared side) — merged superset with `cable` confirmed in `packages/contracts/domain/equipment.ts`; Firestore-side data migration status is external to this repo | Canonical plan §4.5 Step 4 |
| `Result<T>`, `ok()`, `err()` in shared (B.1)        | ✅ Done — confirmed in `packages/contracts/errors/result.ts`                            | Canonical plan §4.4                                                                                |
| `AppErrorCode` superset with scope JSDoc (B.2)       | ✅ Done — confirmed in `packages/contracts/errors/app-error-code.ts`                   | Canonical plan §4.4                                                                                |
| `bodyWeightKgSchema` / `loadWeightKgSchema` split (B.4) | ✅ Done — confirmed in `packages/contracts/schemas/weight.ts`                       | Canonical plan §4.4                                                                                |
| Clay palette (C.1)                                   | ✅ Done — confirmed in `packages/design-tokens/clay.ts`                                | Canonical plan §4.3                                                                                |
| `spacingClay` (C.2)                                  | ✅ Done — confirmed in `packages/design-tokens/spacing-clay.ts`                        | Canonical plan §4.3                                                                                |
| Cloud Functions B.3 cleanup                          | Pending — still in Workouts repo, external to `hollis-shared`                          | This file §B.3                                                                                     |
| `WorkoutSession` deprecated alias removal            | Pending — one-release courtesy, external to `hollis-shared`                            | This file §A.4                                                                                     |
...
| Phase I (`v1.0.0` to GH Packages)                    | Deferred — see `2026-05-13-shared-deps-distribution.md` for current GitHub Packages distribution model | Canonical plan §4.1                                                              |
