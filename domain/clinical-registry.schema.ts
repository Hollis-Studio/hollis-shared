...
export type ClinicalMetricCategory = z.infer<
  typeof ClinicalMetricCategorySchema
>;
...
export const CLINICAL_METRIC_CATEGORY_LABELS: Record<
  ClinicalMetricCategory,
  string
> = {
 ...
 */
export const CLINICAL_METRIC_DIRECTIONS = [
  "lower_better",
  "higher_better",
  "target_range",
] as const;
...
export type ClinicalMetricDirection = z.infer<
  typeof ClinicalMetricDirectionSchema
>;
...
export const CLINICAL_METRIC_DIRECTION_LABELS: Record<
  ClinicalMetricDirection,
  string
> = {
 ...
 */
export const CLINICAL_VALUE_TYPES = [
  "numeric",
  "ratio",
  "qualitative",
  "calculated",
...
export const ClinicalPregnancyStatusSchema = z.enum(
  CLINICAL_PREGNANCY_STATUSES,
);
export type ClinicalPregnancyStatus = z.infer<
  typeof ClinicalPregnancyStatusSchema
>;
...
export const CLINICAL_PREGNANCY_STATUS_LABELS: Record<
  ClinicalPregnancyStatus,
  string
> = {
 ...
 */
export const CLINICAL_MODIFIER_LOGIC_TYPES = [
  "override",
  "offset",
  "multiplier",
] as const;

export const ClinicalModifierLogicSchema = z.enum(
...
export const CLINICAL_MODIFIER_LOGIC_LABELS: Record<
  ClinicalModifierLogic,
  string
> = {
...
export const ClinicalMetricKeySchema = z
  ...
  .regex(
    CAMEL_CASE_PATTERN,
    "Metric key must be camelCase (start with lowercase, no underscores or hyphens)",
  );
 ...
 */
export const ClinicalBaseRangeSchema = z
  .object({
 ...
 */
export const ClinicalOptimalRangeSchema = z
  .object({
  ...
  .refine(
    ...
    {
      message:
        "optimalRange must have at least one of min/max defined, and min must be < max if both are set",
    },
  );
 ...
 */
export const ClinicalHardLimitsSchema = z
  .object({
...
export type ClinicalModifierConditions = z.infer<
  typeof ClinicalModifierConditionsSchema
>;
...
export type ClinicalPopulationModifier = z.infer<
  typeof ClinicalPopulationModifierSchema
>;
...
export type ClinicalPanelComponent = z.infer<
  typeof ClinicalPanelComponentSchema
>;
 ...
 */
export const ClinicalMetricDefinitionSchema = z
  .object({
    // -------------------------------------------------------------------------
    // Core Identity
    // -------------------------------------------------------------------------
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.optimalRange) {
      if (
        data.optimalRange.min !== undefined &&
        data.optimalRange.min < data.baseRange.min
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
      ...
      if (
        data.optimalRange.max !== undefined &&
        data.optimalRange.max > data.baseRange.max
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
...
export type ClinicalMetricDefinition = z.infer<
  typeof ClinicalMetricDefinitionSchema
>;
 ...
 */
export const ClinicalMetricRegistrySchema = z
  .object({
    /**
    ...
    for (const [metricKey, definition] of Object.entries(data.metrics)) {
      ...
      if (definition.parentPanelKey) {
        if (!(definition.parentPanelKey in data.metrics)) {
          ...
          if (
            !parentMetric.panelComponents ||
            parentMetric.panelComponents.length === 0
          ) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
...
export type ClinicalMetricRegistry = z.infer<
  typeof ClinicalMetricRegistrySchema
>;
 ...
 */
export function isClinicalMetricCategory(
  value: string,
): value is ClinicalMetricCategory {
 ...
 */
export function isClinicalMetricDirection(
  value: string,
): value is ClinicalMetricDirection {
 ...
 */
export function isClinicalAgeBracket(
  value: string,
): value is ClinicalAgeBracket {
 ...
 */
export function isClinicalPregnancyStatus(
  value: string,
): value is ClinicalPregnancyStatus {
 ...
 */
export function isClinicalModifierLogic(
  value: string,
): value is ClinicalModifierLogic {
 ...
 */
export function parseClinicalMetricRegistry(
  data: unknown,
): ClinicalMetricRegistry {
 ...
 */
export function parseClinicalMetricDefinition(
  data: unknown,
