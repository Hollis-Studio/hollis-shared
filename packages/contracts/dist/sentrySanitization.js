...
function sanitizeUrlLikeString(value, keyName) {
    const normalizedKey = keyName?.toLowerCase();
    const looksLikeUrl = normalizedKey === "url" ||
...
function shouldStripKey(keyName) {
    const normalizedKey = keyName.toLowerCase();
    return (AUTH_HEADER_NAMES.has(normalizedKey) ||
        SENSITIVE_FIELD_NAMES.has(keyName) ||
        SENSITIVE_FIELD_NAMES.has(normalizedKey));
}
function sanitizeUnknown(value, keyName, seen) {
    ...
    const sanitizedEntries = Object.entries(value).flatMap(([entryKey, entryValue]) => {
        ...
        return [[entryKey, sanitizedValue]];
    });
...
//# sourceMappingURL=sentrySanitization.js.map