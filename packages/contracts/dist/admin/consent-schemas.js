...
export const SignedDocumentPayloadSchema = z.object({
    ...
    signatureDataUrl: z
        .string()
        .min(1)
        .regex(/^data:image\/png;base64,[A-Za-z0-9+/]+=*$/, "Must be a base64 PNG data URL"),
    initialsData: z.record(z.string(), z.string()).optional(),
    optInSelections: z.record(z.string(), z.boolean()).optional(),
    displayedContentHash: z.string().regex(SHA256_HEX_REGEX, "Must be a lowercase 64-character SHA-256 hex digest"),
...
export const EnrollmentSummarySchema = z.preprocess((value) => {
    ...
    return {
        ...summary,
        contractDuration: typeof summary.contractDuration === "number"
            ? summary.contractDuration
            : summary.contractDurationMonths,
        monthlyRateCents: typeof summary.monthlyRateCents === "number"
            ? summary.monthlyRateCents
            : typeof summary.monthlyRateDollars === "number"
                ? Math.round(summary.monthlyRateDollars * 100)
                : summary.monthlyRateCents,
        totalContractCents: typeof summary.totalContractCents === "number"
