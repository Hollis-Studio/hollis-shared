...
export async function validatePasswordStrength(password, userInputs = []) {
    ...
    try {
        ...
        const strengthLabel = ZXCVBN_SCORE_LABELS[result.score];
        ...
        console.warn("[Password] zxcvbn failed to load, falling back to basic validation (dev/test only)");
        return {
            valid: true,
            error: undefined,
        };
...
export async function validatePassword(password, options = {}) {
    const { userInputs = [], checkBreached = PASSWORD_POLICY.HIBP_CHECK_ENABLED, skipStrengthCheck = false, } = options;
...
export function createStrictPasswordSchema(options = {}) {
    return passwordSchema.refine(async (password) => {
