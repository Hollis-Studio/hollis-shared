 ...
 */
export const VoiceLogOperationSchema = VoiceLogOperationRawSchema.superRefine((val, ctx) => {
...
const CardioSlottedExerciseSchema = z
    ...
    .refine((exercise) => exercise.durationSeconds !== null ||
    exercise.targetDistanceKm !== null ||
    exercise.targetSpeedKmh !== null, { message: "Cardio exercises need at least one target" });
/**
 ...
 */
export const SlottedExerciseSchema = z.preprocess((val) => {
...
export const DayRefSchema = z
    ...
    .refine((r) => [r.name, r.index, r.dayOfWeek].filter((v) => v !== undefined).length === 1, { message: "DayRef must specify exactly one of name, index, dayOfWeek" });
...
export const EditOperationSchema = EditOperationRawSchema.superRefine((val, ctx) => {
    ...
    if (val.op === "rename_or_reschedule_day" &&
...
export const SmartNotificationSendResponseSchema = SmartNotificationPreviewResponseSchema.extend({
    delivery: z.object({
...
//# sourceMappingURL=workout-ai-wire.js.map