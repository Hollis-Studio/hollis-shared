...
export function getAppReviewCredentials() {
    const username = (typeof process !== "undefined" ? process.env["APP_REVIEW_USERNAME"] : undefined);
    const password = (typeof process !== "undefined" ? process.env["APP_REVIEW_PASSWORD"] : undefined);
    if (!username) {
        throw new Error("APP_REVIEW_USERNAME environment variable is not set. " +
            "Set it in your .env file (dev) or AWS Secrets Manager (prod).");
    }
    if (!password) {
        throw new Error("APP_REVIEW_PASSWORD environment variable is not set. " +
            "Set it in your .env file (dev) or AWS Secrets Manager (prod).");
    }
