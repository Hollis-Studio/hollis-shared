...
export const MASTER_OFFER_SHEET = MasterOfferSheetSchema.parse(offerSheetData);
export const MASTER_OFFER_TERMS = MASTER_OFFER_SHEET.terms;
export const MASTER_OFFER_ROWS = MASTER_OFFER_SHEET.comparisonRows;
export const MASTER_OFFER_TIER_ORDER = [
    USER_TIER.ESSENTIALS,
    USER_TIER.CORE,
    USER_TIER.CONCIERGE,
];
export function getMasterOfferTier(tier) {
    return MASTER_OFFER_SHEET.tiers[tier];
}
export function getOfferComparisonValue(rowKey, tier) {
    const row = MASTER_OFFER_ROWS.find((candidate) => candidate.key === rowKey);
    return row ? row.values[tier] : null;
}
export function getDiscountedMonthlyPriceDollars(baseMonthlyPriceDollars, discountPercent) {
...
export function getTierCardHighlights(tier) {
    const coachingSessions = getOfferComparisonValue("coachingSessionsPerWeek", tier) ??
        MASTER_OFFER_SHEET.tiers[tier].baseMonthlyPriceDollars.toString();
    const recoveryAccess = getOfferComparisonValue("recoveryAccess", tier) ?? "Included";
    const nutritionCoaching = getOfferComparisonValue("nutritionCoaching", tier) ?? "Included";
    const careCoordination = getOfferComparisonValue("careCoordination", tier) ?? "Included";
    const clinicianVisits = getOfferComparisonValue("clinicianVisits", tier) ?? "Included";
    const labPanels = getOfferComparisonValue("labPanels", tier) ?? "Included";
    return [
...
//# sourceMappingURL=offer-sheet.js.map