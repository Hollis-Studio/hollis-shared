 ...
 */
function offerSheetDiscountPercentForMonths(months: number): number {
  const term = MASTER_OFFER_TERMS.find(
    (candidate) => candidate.months === months,
  );

  if (!term) {
    throw new Error(
      `Master offer sheet has no term for a ${months}-month contract duration. ` +
...
export const CONTRACT_DURATION_DISCOUNTS: Record<ContractDuration, number> =
  ...
  ) as Record<ContractDuration, number>;
...
export type AdminSubscriptionCreateResponse = z.infer<
  typeof AdminSubscriptionCreateResponseSchema
>;

/** Retry returns the same shape as create. */
export const AdminSubscriptionRetryResponseSchema =
  AdminSubscriptionCreateResponseSchema;
...
export type PauseSubscriptionResponse = z.infer<
  typeof PauseSubscriptionResponseSchema
>;
