...
export const SessionExerciseSchema = z
  ...
  .refine(
    ...
    {
      message:
        'canonicalExerciseId must be defined when status is "matched" and null/undefined when "unmatched"',
    },
  );
...
function keepNewest(ids: ReadonlySet<string>): string[] {
  const all = [...ids];
 ...
 */
export function normalizeSessionTombstones(
  ...sources: Array<SessionTombstoneSource | null | undefined>
): SessionTombstones {
  ...
  for (const source of sources) {
    ...
    for (const entry of source.deletedExerciseSlotIds ?? []) {
      if (entry.startsWith(LEGACY_SET_TOMBSTONE_PREFIX)) {
        ...
        if (setId.length > 0) setIds.add(setId);
 ...
 */
export function encodeLegacySessionTombstones(tombstones: SessionTombstones): string[] {
