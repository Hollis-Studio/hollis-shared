...
export async function validatePasswordStrength(
  ...
  try {
    ...
    const strengthLabel =
      ZXCVBN_SCORE_LABELS[result.score as keyof typeof ZXCVBN_SCORE_LABELS];
    ...
    // If zxcvbn fails to load, fail closed in production to prevent weak passwords from slipping through.
    // In development/test, fail open to avoid blocking local workflows.
    if (process.env.NODE_ENV === "production") {
      return {
        valid: false,
        error: "Password strength validation unavailable",
      };
    }
    console.warn(
      "[Password] zxcvbn failed to load, falling back to basic validation (dev/test only)",
    );
    return {
      valid: true,
      error: undefined,
...
export async function checkPasswordBreached(
  ...
  try {
    ...
    const timeoutId = setTimeout(
      () => controller.abort(),
      PASSWORD_POLICY.HIBP_TIMEOUT_MS,
    );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(
          "[Password] HIBP API returned non-OK status:",
          response.status,
        );
 ...
 */
export function createStrictPasswordSchema(
  options: {
    userInputs?: string[];
    checkBreached?: boolean;
  } = {},
  ...
  return passwordSchema.refine(
    ...
    {
      message:
        "Password does not meet security requirements. Use a stronger, unique password.",
    },
  );
}
